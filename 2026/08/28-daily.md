...
- **WikiSkill：将智能体经验编译为持久知识以实现技能进化**
  *WikiSkill: Compiling Agent Experience into Persistent Knowledge for Skill Evolution*
  ...
  📝 **摘要**：本文提出WikiSkill框架，旨在通过共进化智能体技能与持久知识库（Wiki）来解决现有技能发现方法中经验分散、难以系统复用的问题。该框架将原始执行经验、累积知识和可执行技能分离，并持续将经验整合至Wiki中，供后续技能更新构建。实验表明，WikiSkill在多种基准测试和模型上均优于最先进技能进化方法及无技能基线。研究发现技能进化与模型规模互补：大模型从进化技能中获益更多，而配备技能的小模型可超越无技能的大模型。此外，进化技能在不同模型及家族间具有良好迁移性，甚至由其他模型进化的技能可能优于自进化技能。消融实验证实，Wiki中的持久知识积累是有效技能进化的关键。
...
- **SWE-Prime：更少轨迹，更好性能**
  *SWE-Prime: Fewer Trajectories, Better Performance*
  ...
  📝 **摘要**：针对大语言模型解决真实软件问题时，成功轨迹中可能包含无效、冗余或高风险步骤导致监督信号噪声的问题，本文提出SWE-Prime，一种多粒度两阶段SFT数据选择方法。第一阶段基于过程质量、结果质量和数据代表性进行轨迹级筛选，选取高质量且具代表性的成功轨迹子集；第二阶段将连续步骤分组为语义段，根据其对最终解决方案的贡献、可学习性及潜在风险进行段级选择。在SFT过程中，所有段保留以维持上下文，但仅选中的段参与损失计算。在SWE-Bench Pro和Verified上的实验显示，使用SWE-Prime筛选出的10%轨迹子集进行训练，其性能优于使用完整解决数据集训练，相对性能提升分别高达12.2%和24.2%。
...
- **从静态到动态：使用MCR-Bench对真实世界代码审查进行基准测试**
  *From Static to Dynamic: Benchmarking Real-World Code Review with MCR-Bench*
  ...
  📝 **摘要**：现有代码审查方法常将其简化为单轮静态决策任务，无法捕捉真实场景中多轮交互的复杂性。为此，本文提出MCR-Bench，首个面向真实多轮代码审查的缺陷状态感知基准。该基准涵盖五种常用编程语言，包含2,269个真实多轮代码审查任务，每个任务均标注细粒度缺陷信息（如描述、类型、严重性）及跨轮次状态标签，以捕捉缺陷在多轮过程中的完整演化轨迹。对主流LLM的大量实验揭示了三点发现：(1) LLM在缺陷检测和生命周期状态跟踪方面整体能力有限，且随交互轮次增加性能显著下降；(2) 性能对缺陷类型和严重性敏感，语义复杂或低显著性缺陷更易被遗漏；(3) 错误分析显示跨轮次时间错位和长程记忆不足是导致误报和漏报的关键弱点。
...
- **RedEvoAgent：基于经验驱动技能进化的自动红队智能体**
  *RedEvoAgent: Automatic Red-Teaming Agent with Experience-Driven Skill Evolution*
  ...
  📝 **摘要**：针对LLM智能体在产品级执行框架中面临越狱攻击风险，且现有自动红队方法存在检索偏差和工具信用不明等问题，本文提出RedEvoAgent，一种黑盒红队智能体。该方法将跨案例攻击轨迹蒸馏为简洁、人类可读的攻击技能，并通过工具有效性画像和决策工具归因（Deciding-Tool Attribution）自适应进化技能，同时利用验证棘轮机制仅保留能提升验证性能的更新。在多个基准、目标模型和执行框架上的实验表明，RedEvoAgent优于固定和智能体基线方法，提高了工具效率，并能在不同攻击模型和目标执行框架间有效迁移。该工作展示了经验驱动技能进化在自动化安全评估中的潜力。
...
- **基于图结构电子占据离散流匹配的机理反应预测**
  *Mechanistic Reaction Prediction via Discrete Flow Matching on Graph-Structured Electron Occupation*
  ...
  📝 **摘要**：现有机器学习方法通常通过从头生成产物分子或启发式图编辑来建模化学反应，缺乏对电子空间变换的机理理解。本文提出MAELLE（MechAnistic Edit fLow-matching on eLectron rEarrangements），将反应建模为电子占据向量上的离散流匹配。具体而言，MAELLE将反应物到产物的映射定义为定义在所有成键、非成键和氢位点上的图结构整数电子占据空间上的连续时间马尔可夫链（CTMC）。通过利用最优传输泛化离散流匹配混合路径，MAELLE生成无需基元步骤标注的机理可解释编辑移动序列。在USPTO-480K基准上，MAELLE表现出与领先模型相当的竞争力，且在结构复杂性和反应类型等分布外设置中保持稳健性能。此外，MAELLE能自然恢复与已知化学一致的机理轨迹并预测副产物。
...
- **人格-执行分离：执行审计下进化LLM智能体的架构模式**
  *Persona-Execution Separation: An Architecture Pattern for Evolving LLM Agents under Execution Audit*
  ...
  📝 **摘要**：在受治理的组织中，LLM智能体需允许人格（指令、语气等）自由进化，同时保持执行（有状态、可审计的工作）的可追溯性。本文提出人格-执行分离（PES）架构模式，将人格和执行置于不同的信任域，通过受治理的合同桥连接。人格可漂移，执行则无面且受审计；状态摘要可返回，但数据主体保留在受限域，除非有分级数据丢失预防（DLP）例外。PES基于自由漂移、执行可追溯性和解耦三个目标推导而来。在受监管数字员工平台的开发/试点案例中，PES记录了五个月内的决策，机制检查证实执行侧在人格扰动下无重新验证，且硬断言字段无人格指纹。该模式适用于多用户部署、执行审计和预期人格频繁变更同时存在的场景，确保了架构层面的审计规则。
...
- **超越F1：评估AI模型安全扫描器的覆盖率与故障恢复能力**
  *Beyond F1: Evaluating Coverage and Failure Recovery in AI Model Security Scanners*
  ...
  🏛️ **单位**：eBay Inc.
  📝 **摘要**：传统评估指标仅关注扫描器产生可用安全判断的情况，忽略了覆盖率和故障恢复。本文构建了一个包含170个Pickle和PyTorch工件、145个样本族的受控基准，其中135个具有二元安全真值，10个为故意畸形无标签。评估了ModelScan、ModelAudit和Fickling三个扫描器，明确区分非N/A覆盖率、分析完成度、确定性安全决策等。结果显示，ModelAudit对135个有标签族均产生确定性决策（100%），Fickling为81.5%，ModelScan仅为49.6%。在做出确定性判断的条件下，ModelScan的精确率、召回率和F1均为100%。Fickling未识别出ModelAudit和ModelScan组合之外的唯一真阳性族。对于ModelScan分析失败的48个恶意族，ModelAudit和Fickling均生成了与真值一致的检测。这表明需将判断准确性与可用性、增量检测覆盖率与工具级冗余分开评估。
...
- **无需逐小时监督学习连续脓毒症严重程度评分：一项双中心回顾性研究**
  *Learning a Continuous Sepsis Severity Score Without Hour-by-Hour Supervision: A Two-Site Retrospective Study*
  ...
  📝 **摘要**：现有脓毒症严重程度指数依赖数十年前的固定变量和权重，无法反映当代重症监护。本文开展了一项双中心回顾性研究，利用来自马萨诸塞州和佐治亚州两家医院系统的29,116名和7,691名符合Sepsis-3标准的成年患者数据，开发了一种基于72小时治疗窗口内43个常规记录变量的脓毒症指数。不同于以往研究，该方法将死亡率作为治疗级排序信号而非逐状态目标，允许信用在非均匀时间步长间重新分配。在20%永久测试集上，非幸存者在所有基线SOFA-2分层中得分比幸存者高1.19-1.64分（0-10分制）。指数变化与乳酸变化相关（Spearman rho = 0.39）。跨机构一致性显示，不同站点训练的模型间Spearman相关系数为同站点相关性的70-77%。该指数展示了每小时预后信息，有望作为辅助临床判断的决策支持工具。
...
- **CLAP：跨具身视频世界模型是零样本物理模拟器**
  *CLAP: Cross-Embodiment Video World Models are Zero-Shot Physical Simulators*
  ...
  📝 **摘要**：现有动作条件视频模型通常局限于单一机器人具身，无法利用异构视频数据学习通用物理。本文提出CLAP，一个跨具身动作条件视频生成框架，可训练于包含人类和机器人代理的互联网规模多样化视频。CLAP基于普遍物理定律支配时空动态的洞察，通过末端执行器位姿、语言指令和潜在动作协调不同的动作空间。引入基于课程的跨具身学习策略，首先利用潜在动作在无标签视频数据中学习基础物理先验，随后将其接地到末端执行器动作空间以实现零样本部署。在DROID等挑战性环境中，CLAP接近或超越最先进单具身视频模型，并通过少样本适应建立训练单具身视频世界模型的新范式。CLAP提供了迄今最全面的动作条件视频世界模型套件，涵盖多种动作条件空间和机器人形态，所有代码和模型已开源。
...
- **语言模型如何组织和结构化道德知识**
  *How Language Models Organize and Structure Moral Knowledge*
  ...
  📝 **摘要**：本文探讨大型语言模型（LLM）如何组织道德知识，超越简单的道德内容检测。通过在开源语言模型上训练六个独立的线性探针（对应道德基础理论MFT的六个类别），研究表示空间中这些方向的关系。发现这些方向既未坍缩为单一道德检测器，也未彼此隔离，而是跨越近最大数量的独立维度，同时共享一个正向公共组件。该共享组件是整合的标志，且相对于匹配的非道德概念电池具有道德特异性（平均成对余弦0.26 vs 0.013）。这种几何结构在不同架构和规模间保持一致，并在预训练早期达到整合状态。模型发现的结构未显示MFT预测的个体化/结合区分，而是反映语料库统计。在道德困境中，每个困境方向部分由其组成基础构成，模型代表的是道德张力本身，而非预先解决的判断。
  🔗 [PDF](https://arxiv.org/pdf/2608.27402v1)

- **使临床语言模型可审计：概念引导微调以实现稳健预测**
  *Making Clinical Language Models Auditable: Concept-Guided Fine-Tuning for Robust Prediction*

  📄 `arXiv:2608.27397` · cs.CL, cs.AI
  👥 **作者**：Jin Mu, Guanhua Chen
  🏛️ **单位**：University of Wisconsin–Madison
  📝 **摘要**：临床语言模型虽在医院内准确率高，但因利用笔记特定伪影（如模板、分隔符）而在部署偏移时失效。本文提出CAST（Concept-guided Artifact Suppression Tuning），一种基于SAE的可审计临床文本分类框架。CAST利用稀疏自编码器（SAE）从Transformer中间激活中暴露稀疏、人类可审计的特征，通过LLM辅助解释管道和ICD-10检索约束对SAE潜在变量进行标注，并在微调期间通过残差减法抑制已验证的伪影潜在变量，同时提供事后逐概念归因以审计模型决策。在MIMIC-IV出院笔记死亡率预测任务上，CAST优于相应的微调编码器基线，并与强大的LLM基线保持竞争力，同时生成支持每个预测的临床概念及训练期间被抑制的伪影概念的特征级审计轨迹。
...
- **LeVJEPA：无启发式的高效可扩展视频预训练**
  *LeVJEPA: Efficient & Scalable Video Pretraining without the Heuristics*
  ...
  📝 **摘要**：视频承载物理世界的时序结构，但现有自监督方法计算昂贵，依赖架构不对称或像素空间重建来防止表示坍缩。本文提出LeVJEPA，首个在LeJEPA无坍缩目标下训练的视频编码器，摒弃了上述启发式方法。LeVJEPA使用单个编码器，通过全局和局部视图的不变性损失训练，并由SIGReg正则化，以可证明的保证排除坍缩。架构简化为编码器和投影器，目标简化为单一超参数。实验表明，在相同数据下，LeVJEPA以5.6至20.8倍更少的预训练计算量匹配或超越V-JEPA 2。由于无需分支不对称，编码器可使用块因果注意力训练，使时序顺序成为编码器本身的属性。与计算匹配的DINOv2相比，LeVJEPA在外观中心评估上接近图像预训练编码器，而在运动中心评估上准确率几乎翻倍。这表明视频是通用视觉预训练的可行且优越的基底。
  🔗 [PDF](https://arxiv.org/pdf/2608.27395v1)

- **异构观测条件下从接触式PPG到相机rPPG的属性特定可恢复性**
  *Property-Specific Recoverability from Contact PPG to Camera rPPG under Heterogeneous Observation Conditions*
  ...
  🏛️ **单位**：Department of Computer Science, Morgan State University
  📝 **摘要**：相机衍生远程光电容积脉搏波（rPPG）通常通过端点准确性验证，但这不能确立源接触式PPG的其他生理属性是否逐记录保留。本文在Multi-Domain Mobile Video Physiology Dataset的655个记录上评估了属性特定的PPG到rPPG可恢复性，使用CHROM作为固定相机rPPG观测路径。结果显示，最大Lyapunov指数在记录特定PPG-rPPG对应关系上几乎无差异（相关系数0.0231），尽管存在群体水平重叠。端点差异在调整光照和运动后表现出Fitzpatrick相关的异质性（Fitzpatrick VI vs III对比为9.32 bpm），而动态差异未显示相应梯度。在主体留出分析中，添加运动和光照一致地降低了线性、岭和随机森林学习器的MAE，降幅高达13.32%。这些发现表明可恢复性是属性特定的，群体水平的合理性不能确立个体记录的保留。
...
- **CorporateBench：基于时序知识库的大规模问答基准测试**
  *CorporateBench: Large-Scale Q&A Benchmarking with Temporal Knowledge Bases*
  ...
  📝 **摘要**：LLM日益能够回答关于企业级文档集合的复杂问题，但评估困难：公司不愿分享内部通信，合成数据集过于简单。本文提出CorporateBench（CB），一个经人工验证的多任务问答基准，其规模接近LLM在企业通信网络中遇到的条件，评估语料库超过230,000份文档。CB通过四个合成生成的公司（员工数从12到10,000不等）在两个维度（信息提取和知识库查询）上评估LLM。每个语料库从描述一致世界的时序演化知识库中采样，保证即使跨越数十万份文档，跨文档逻辑一致性依然存在。对五个LLM的评估揭示，随着输入规模接近现实水平，性能逐渐下降。CB为LLM开发者提供了企业通信推理的指标，填补了基准测试生态系统中的关键空白。
...
- **连续容量增长：JEPA世界模型中视觉Transformer编码器的任务复杂度驱动宽度与深度扩展**
  *Successive Capacity Growth: Task-Complexity-Driven Width and Depth Expansion for Vision Transformer Encoders in JEPA World Models*
  ...
  📝 **摘要**：用于世界建模的联合嵌入预测架构（JEPA）通常使用固定大小的视觉Transformer编码器，这对简单任务过度配置，对复杂任务配置不足。本文提出连续容量增长（SCG）方法，从最小编码器（1头，2层，283K参数）开始，根据任务无关的测试与验证机制，在宽度（添加注意力头）或深度（添加Transformer块）上增量增长。该机制利用函数保持扩展安全地尝试架构变更，若未改善预测损失则回滚。SIGReg确保所有学习到的语义维度保持统计独立并对齐预测目标，防止架构增长时的坍缩。在60维多物体动力学任务上，SCG自然触发深度扩展，预测损失比固定小基线改善20.3%，参数效率比固定大模型高56倍；在2D导航任务上，单次宽度扩展比固定大模型改善23%。结果表明JEPA编码器无需预分配最大容量，可随任务需求连续增长，实现计算和数据效率。
...
- **无需逐小时监督学习连续脓毒症严重程度评分：一项双中心回顾性研究**
  *Learning a Continuous Sepsis Severity Score Without Hour-by-Hour Supervision: A Two-Site Retrospective Study*
  ...
  📝 **摘要**：现有脓毒症严重程度指数依赖数十年前的固定变量和权重，已无法反映当代重症监护现状。本研究基于马萨诸塞州和佐治亚州两家医院系统的36,807名符合Sepsis-3标准的成年患者，开发了一种基于72小时治疗窗口内43个常规记录变量的连续脓毒症指数。不同于以往研究，该方法将死亡率作为治疗层面的排序信号而非逐状态目标，允许信用在非均匀的时间步长间重新分配。在20%的永久测试集上，非幸存者在所有基线SOFA-2分层中得分比幸存者高1.19-1.64分（0-10分制），且在乳酸、平均动脉压和肌酐分层中表现一致。指数变化与乳酸变化呈显著相关（Spearman rho = 0.39）。跨机构一致性达到同机构相关的70-77%。该指数展示了每小时预后信息，能有效区分患者结局，具有作为临床决策支持工具的潜力。
  🔗 [PDF](https://arxiv.org/pdf/2608.27421v1)
...
- **扩展图神经网络用于好友推荐：多哈希用户嵌入与时序邻居采样**
  *Scaling Graph Neural Networks for Friend Recommendation: Multi-Hash User Embeddings and Temporal Neighbor Sampling*
  ...
  🏛️ **单位**：AI VK
  📝 **摘要**：好友推荐本质上是图结构问题，但部署消息传递GNN于生产级社交图谱（数亿用户、数十亿边）面临建模和系统挑战。本文提出一种可扩展的端到端GNN排序系统，重点解决两个关键设计选择：多哈希ID嵌入和时序邻居采样。多哈希嵌入作为主要节点表示，将ID嵌入表大小减少98%以上，同时保持排序质量。针对拥有数万好友的用户，实现基于时间戳排序CSR存储和二分查找的时序邻居采样，将单节点采样成本从O(deg(v) + k)降低至O(log(deg(v)) + k)。在包含1.94亿用户和280亿边的图谱上，离线消融实验验证了各设计选择的贡献。在线A/B测试显示，该系统相比强生产基线，好友添加量增加16%，唯一好友添加者增加11.5%。作者发布了用于大型时序图分布式训练和推理的框架。
...
- **语言模型如何组织和结构化道德知识**
  *How Language Models Organize and Structure Moral Knowledge*
  ...
  📄 `arXiv:2608.27402` · cs.CL, cs.AI, cs.LG
  👥 **作者**：Orion Reblitz-Richardson
  📝 **摘要**：本文探究大型语言模型（LLMs）如何组织道德知识，超越简单的道德内容检测。通过在开源语言模型上训练六个独立的线性探针（对应道德基础理论MFT的六个类别：关怀/伤害、公平/欺骗、自由/压迫、忠诚/背叛、权威/颠覆、圣洁/堕落），研究表示空间中这些方向的关系。发现这些方向既未坍缩为单一道德检测器，也未彼此隔离，而是跨越近最大数量的独立维度，同时共享一个正向公共组件。该共享组件是整合的标志，且相对于匹配的非道德概念电池具有道德特异性（平均成对余弦0.26 vs 0.013）。这种几何结构在架构和规模间保持一致，并在预训练早期达到整合状态。模型发现的道德结构未显示MFT预测的个体化/结合区分，而是反映语料库统计特性。在道德困境中，每个困境方向部分由其组成基础合成，模型代表的是道德张力本身，而非预先解决的判断。
  🔗 [PDF](https://arxiv.org/pdf/2608.27402v1)
...
- **CorporateBench：基于时序知识库的大规模问答基准测试**
  *CorporateBench: Large-Scale Q&A Benchmarking with Temporal Knowledge Bases*
  ...
  👥 **作者**：Sil Hamilton, Albert Yu Sun, Oscar J. Romero, Carl-Leander Henneking, David Mimno, Bishan Yang, Igor Labutov
  🏛️ **单位**：Epiq AI Labs, Cornell University
  📝 **摘要**：随着LLM在回答企业级文档集合复杂问题方面能力增强，评估变得困难：公司不愿分享内部通信，合成数据集过于简单。本文提出CorporateBench（CB），一个经人工验证的多任务问答基准，其规模接近LLM在企业通信网络中遇到的条件，评估语料库超过230,000份文档。CB通过四个合成生成的公司（员工规模从12到10,000人）在两个维度（信息提取和知识库查询）上评估LLM。每个语料库从描述一致世界的时序演变知识库中采样，保证即使跨越数十万份文档的跨文档逻辑一致性。对五个LLM的评估揭示，随着输入规模接近现实水平，性能逐渐下降。CB为LLM开发者提供了企业通信推理的度量标准，填补了基准测试生态系统中的关键空白。
  🔗 [PDF](https://arxiv.org/pdf/2608.27391v1)
...
- **Token级广告**
  *Token-Level Advertising*
  ...
  📝 **摘要**：生成式AI正在改变人们获取信息的方式，挑战基于预定义广告位的传统广告机制。面向生成原生广告，本文提出潜在广告主混合拍卖（LAMA），一种将广告主影响直接嵌入生成过程的Token级广告机制。广告主报告局部延续值，诱导广告主特定的下一Token策略，平台通过潜在混合解码并更新分配后验。证明LAMA满足马尔可夫DSIC和IR性质，并实现近最优的KL正则化福利。进一步开发基于学习的实现，从学习的局部优势和根值在线重建所需报告。在真实商业搜索查询分割上的概念验证实验表明，LAMA在保持用户端响应质量的同时，提高了平台福利和收入，为生成原生广告的可行性提供了初步证据。
...
- **椭球拟合的普适性与尖锐阈值**
  *Universality and sharp thresholds for ellipsoid fitting*
  ...
  📝 **摘要**：本文建立了用椭球拟合随机向量的尖锐相变。随机向量具有独立的亚高斯坐标，均值为零，方差为一，且共有第四矩，向量数量与维度平方成正比。识别出显式的可满足性阈值：低于阈值时，以高概率存在正定椭球穿过每个数据点；高于阈值时，不存在半正定拟合。还确定了不可满足区域中最佳平方拟合误差。特别地，阈值仅通过坐标分布的共同第四矩依赖，揭示了第四矩普适性现象。对于标准高斯数据，阈值为1/4，解决了椭球拟合猜想。
  🔗 [PDF](https://arxiv.org/pdf/2608.27372v1)

- **Puro-2B：Poor Lab在RTX 5090上以5090美元以内训练Qwen2-1.5B**
  *Puro-2B: Poor Lab's Qwen2-1.5B Trained on RTX 5090 within $5090*
  ...
  📝 **摘要**：语言模型预训练通常与高昂成本挂钩，使学术界和开源社区难以触及。本文提出一种开源预训练配方，旨在降低这一门槛。使用该配方，在消费级RTX 5090 GPU上以FP8精度从头训练Puro-2B模型系列，处理多达1.4万亿Token。最佳模型训练成本低于6,900美元，在评估协议下接近Qwen2.5-1.5B性能。这种成本效率由硬件选择、低精度训练、超球优化、课程模型平均和数据配方共同实现。此外，推导了Puro成本缩放定律，表明约4,400美元即可达到Qwen2-1.5B性能。作为端到端案例研究，考察预训练数据课程如何塑造后训练后的下游性能。完整训练配方（包括数据、代码和模型权重）以Apache 2.0协议发布。
...
- **理解LLM推理的进化策略：比GRPO更广泛的推理覆盖**
  *Understanding Evolution Strategies for LLM Reasoning: Broader Reasoning Coverage than GRPO*
  ...
  📝 **摘要**：进化策略（ES）近期成为LLM推理的记忆高效后训练范式，但其优化行为研究不足。本文系统研究ES动态和机制，理论上和实证上证明ES可导致更广泛的推理覆盖，从而更好地利用预训练LLM的推理能力。理论上，验证器投影的Jensen-Shannon多样性有助于更高的Pass@K性能。实证上，与表现出熵坍缩的GRPO不同，ES在提高Pass@1的同时获得比GRPO更高的Pass@K。进一步开发顺序GRPO-ES训练策略，结合GRPO在Pass@1的优势和ES在Pass@K的收益。发现尽管整体模型参数漂移显著，ES的任务性能增益仅由稀疏的大幅度更新子集贡献，这种功能稀疏性表明大参数移动不一定意味着广泛的功能变化，也不一定导致灾难性遗忘。研究还表明ES在更大LLM中需要更小的种群规模。
...
- **超越并行盲点：块草稿中的信息下限与模型差距**
  *Beyond Parallel Blindness: Information Floors and Model Gaps in Block Drafting*
  ...
  📝 **摘要**：块草稿器在一次前向传递中提出多个Token，此时早期目标Token尚未实现。其拒绝混合了两种损失：缺失块内路径信息和不完美的可观察信息建模。接受长度无法区分这两者。本文通过信息下限（指定条件顺序下的最小预期拒绝）分离两者，超过此下限的拒绝即为模型差距。在四个领域、四个开源权重目标和前沿API目标上估计两者，得出三个发现：首先，在Qwen3-4B上，全并行下限在最终槽位达到0.286，限制最佳提案的每槽接受率为71%；其次，一个已实现Token移除此下限的86-100%，这种局部性也被独立互信息分析恢复；第三，当前草稿器远高于其下限：最终槽位模型差距占DFlash拒绝的43-64%和DSpark oracle条件拒绝的85-92%。这些发现分离了短程条件的价值与提案质量。
...
- **分布强化学习中分位数时序差分学习的有限样本分析**
  *A Finite Sample Analysis for Quantile Temporal Difference Learning in Distributional Reinforcement Learning*
  ...
  📝 **摘要**：本文建立了表格分布强化学习中同步分位数时序差分学习（QTD）的全局有限样本保证。证明分离了两种稳定性机制：基于奖励累积分布函数顺序单调性和分布Bellman算子W∞收缩的全局比较论证，将任意初始化的迭代带入局部邻域；在该邻域内，线性化QTD平均场，其Jacobian是非奇异M-矩阵，相关的正半群允许方差敏感的鞅分析。对于步长α_t=c(t+1)^{-a}（a∈(1/2,1)），主导的最后迭代波动为O~(T^{-a/2}/√(1-γ))，且对分位数数量无多项式依赖。确定性瞬态和所需预热期仍可能依赖最小的Bellman目标密度，最坏情况下为m^{-1}。该结果清晰区分了局部随机波动和全局样本复杂度。
...
- **QuantumBoostNet：用于提高心脏超声视图识别精度的混合经典-量子架构**
  *QuantumBoostNet: A Hybrid Classical-Quantum Architecture for Enhanced Accuracy in Cardiac Ultrasound View Identification*
  ...
  🏛️ **单位**：Politehnica University Timișoara, University Hospital Münster
  📝 **摘要**：准确识别心脏超声（超声心动图）中的正确视图或角度是心脏成像的关键组成部分，对精确解剖解释、可靠测量和减少临床错误至关重要。尽管计算机视觉进步显著，但大多数最先进模型在标准基准上表现良好，却因数据高噪声而在专业医学成像任务中产生次优结果。本文引入QuantumBoostNet，一种混合经典-量子架构，整合经典骨干和两个头部：一个经典和一个量子，量子头实现为参数化10量子比特量子电路。训练分两个阶段进行，由监控损失动态的混合参数控制头部间的自适应转换。大量实验表明，尽管可模拟量子比特数量有限，QuantumBoostNet在心脏超声视图识别中持续优于最先进经典和混合经典-量子模型，相比最佳竞争者取得相对改进。该模型在既定图像分类基准上也表现出优越性能和对噪声的鲁棒性，支持混合经典-量子模型在专业医学成像应用中的持续发展。
...
- **LLM可以设计近最优运筹学算法**
  *LLMs Can Design Near-Optimal OR Algorithms*
  ...
  👥 **作者**：Jackie Baek
  📝 **摘要**：本文探究大型语言模型（LLMs）能否为明确定义的运筹学（OR）问题设计有效算法。研究库存控制、排队网络控制和组合优化问题。评估两个LLM使用层级：层级1中，模型接收一个问题实例并返回该实例的解；层级2中，仅接收问题类描述和宽泛参数范围，返回将实例参数映射到解的算法。人类输入最小化：提供一个未调优的问题描述提示，模型可访问具有固定计算预算的Python沙箱工具。测试的最强模型gpt-5.6-sol在几乎所有评估实例上匹配或超越现有最佳方法，即使在层级2中（返回的算法在看到评估实例前固定）。性能在发布间隔不到八个月的模型间显著改善，表明该能力正在快速进步。对于研究的明确定义运筹问题，单次未调优LLM查询已能产生与专门方法竞争的算法，表明前沿LLM可作为明确定义OR问题算法设计的严肃实证基线。
...
- **从声学分布中恢复音乐艺术家间专家评论家来源的网络邻接性：一种构念效度方法**
  *Recovering Expert Critic-Sourced Network Adjacency between Musical Artists from Acoustic Distributions: A Construct-Validity Approach*
  ...
  📝 **摘要**：音乐推荐主要依赖两种信号：用户-项目交互（在冷启动中失效）和内在音乐内容。本文认为第三种未被充分利用的信号更丰富且更有原则：评论邻接性，即专家评论家长篇散文中明确链接两位艺术家的成对关系，编码了关于哪些艺术家应归为一类的刻意判断。先前工作建立了其内部效度，但缺乏外部验证：这种评论家来源的关系是否基于音乐本身而非社会学背景。本文通过声学内容进行测试，将问题重构为构念效度。将艺术家表示为80个低级Essentia声学描述符上的经验分布，通过边际最优传输（Wasserstein）距离建模成对接近度，在冷启动、艺术家不相交分割下评估评论邻接性的声学可恢复性。集成模型以0.767的样本外AUC（95% CI 0.761-0.775）恢复这些边。可恢复性随评论共识单调上升，在多源证实边上达到0.865。分层评估与社会学流派模型一致：紧密界定的场景基础流派比宽泛的行业伞形术语具有更高的可恢复性。评论话语是推荐的丰富信息来源，可分解为可重现的“声学核心”和由叙事定位、亚文化背景和经典地位驱动的“社会学剩余”。
...
- **MM-Spectrum：基于稳定MoE框架的多模态多光谱分子结构解析**
  *MM-Spectrum: Multimodal Multi-spectral Molecular Structural Elucidation with a Stable MoE Framework*
  ...
  📝 **摘要**：从多模态光谱测量推断分子结构需要整合互补但高度异质的信号。然而，直接拼接多光谱序列的常见范式可能表现出异常的性能退化，主要由于显著的异质性和由此导致的模态间多模态不平衡。作为补救，本文提出MM-Spectrum，一种专为多模态多光谱谱-结构解析设计的稀疏混合专家（MoE）框架。为更好地匹配多光谱不平衡下的信息特征，MM-Spectrum引入显式的模态感知路由机制，除Token内容表示外，还将光谱身份暴露给路由器。此外，它整合共享和交互专家，以及异质专家容量，以提取多光谱模态独特和跨模态协同信息，同时抑制噪声引起的干扰。在分子结构解析的全模态、双模态和缺失模态设置中，MM-Spectrum取得一致且显著的改进，得到消融研究和可解释性分析的支持。
  🔗 [PDF](https://arxiv.org/pdf/2608.27286v1)

- **使潜在演化显式化：世界动作模型的算子结构化转换**
  *Making Latent Evolution Explicit: Operator-Structured Transitions for World Action Models*
  ...
  📝 **摘要**：世界动作模型（WAMs）通过预测任务相关场景状态在交互下如何演化来增强机器人策略。近期WAMs越来越多地在潜在表示空间中进行此类预测，避免完整外观级生成同时保留控制相关信息。然而，潜在转换通常用基于Transformer的预测器实现，其归纳结构以Token交互为中心而非时间演化。本文将转换实现研究为区别于预测表示和预测-策略耦合的架构选择。引入潜在演化算子网络（LEON），通过学习到的可观察空间中的上下文调制算子传播和加性强制来建模潜在演化。基于受控Koopman生成器的演化观点，LEON围绕共享演化算子结构组织上下文依赖的转换变化，同时保留加性变化的互补路径。受控动力系统验证了由此产生的演化特定归纳偏差以及算子传播和强制的互补作用。在两种以不同方式将潜在预测整合到策略中的WAM公式中，LEON提高了闭环性能和鲁棒性，同时在完全转换替换下保持有效。这些结果确立了转换实现是潜在WAMs中重要的架构选择。
...
- **CritICL：基于小语言模型失败模式的推理时弱到强泛化**
  *CritICL: Inference-Time Weak-to-Strong Generalization from Small Language Model Failure Modes*
  ...
  📝 **摘要**：针对现有推理时扩展方法依赖重复生成或外部验证导致的高成本问题，本文提出CritICL框架。其核心洞察在于同一模型家族中，大语言模型的失败模式在不同规模间呈现结构化规律。CritICL不再将失败视为无效输出，而是利用弱模型的失败模式作为指导信号，通过基于批评的上下文示例（critique-based in-context examples）融入推理过程。论文提出了动态（CritICL-dynamic）和静态（CritICL-static）两种变体，分别自适应预测特定输入的失败模式或使用全局失败模式画像。实验表明，CritICL在保持高推理效率的同时，性能优于标准上下文学习，并与测试时扩展方法相当或更优，显著降低了生成次数和Token成本。
...
- **WikiSkill：将智能体经验编译为持久知识以实现技能进化**
  *WikiSkill: Compiling Agent Experience into Persistent Knowledge for Skill Evolution*
  ...
  👥 **作者**：Liyan Tang, Cyrus Rashtchian, Chun-Sung Ferng, Andrew Tomkins, Da-Cheng Juan, Tu Vu
  🏛️ **单位**：Google Research, Virginia Tech
  📝 **摘要**：现有智能体技能发现方法中，指导技能发展的洞察往往分散在优化历史中，限制了系统性复用。本文提出WikiSkill框架，通过让智能体技能与持久知识库（Wiki）共同进化来解决这一问题。WikiSkill将原始执行经验、累积知识和可执行技能分离，并持续将经验整合到Wiki中，供后续技能更新构建。实验显示，WikiSkill在多种基准和模型上均优于最先进技能进化方法。研究发现技能进化与模型扩展互补：大模型从进化技能中获益更多，而拥有技能的小模型可超越无技能的大模型。此外，进化技能在不同模型间具有良好迁移性，且由其他模型进化的技能有时优于自进化技能。消融实验证实，Wiki中的持久知识积累对有效技能进化至关重要。
  🔗 [PDF](https://arxiv.org/pdf/2608.27454v1)
...
- **SWE-Prime：更少的轨迹，更好的性能**
  *SWE-Prime: Fewer Trajectories, Better Performance*
  ...
  👥 **作者**：Dewu Zheng, Ruizhe Ye, Yanlin Wang, Yang Ye, Hongyu Zhang, Ensheng Shi, Xilin Liu, Yuchi Ma, Jianxing Yu, Zibin Zheng
  🏛️ **单位**：Sun Yat-sen University, Huawei Cloud Computing Technologies Co., Ltd., Chongqing University
  📝 **摘要**：针对软件问题求解中成功轨迹可能包含无效或冗余步骤导致监督噪声的问题，本文提出SWE-Prime，一种多粒度、两阶段的监督微调（SFT）数据选择方法。第一阶段基于过程质量、结果质量和数据代表性进行轨迹级筛选，选取高质量且具代表性的成功轨迹子集。第二阶段进行片段级选择，将连续步骤分组为语义片段，并评估其对最终解决方案的贡献、可学习性及潜在风险。在SFT过程中，所有片段保留以维持上下文，但仅选中的片段参与损失计算。在SWE-Bench Pro和Verified上的实验表明，使用SWE-Prime筛选出的10%轨迹子集进行训练，性能优于使用完整解决数据集训练，相对性能提升最高达12.2%和24.2%。
  🔗 [PDF](https://arxiv.org/pdf/2608.27449v1)
...
- **TTPO：测试时策略优化**
  *TTPO: Test-Time Policy Optimization*
  ...
  📝 **摘要**：现有后训练方法如强化学习（RL）和在线策略自蒸馏（OPSD）依赖真实标签，限制了测试时训练（TTT）的应用。本文观察到多数投票伪标签的错误具有不对称性：与伪标签不一致的采样结果通常本身就是错误的。基于此，提出测试时策略优化（TTPO），一种非对称目标函数，通过OPSD蒸馏一致采样，并通过分组RL惩罚不一致采样。Token级选择进一步细化分支：蒸馏降低已收敛位置的权重，RL仅惩罚高置信度错误。即使伪标签频繁出错，TTPO仍能保持稳健。实验显示，TTPO在无标签情况下匹配有标签OPSD在五个竞赛级基准上的性能，将Qwen3-1.7B在TTT中的准确率从38.0%提升至45.2%，并在非思考模式下获得25.2%至36.4%的提升，展现出强大的跨任务泛化能力。
...
- **从静态到动态：使用MCR-Bench对真实世界代码审查进行基准测试**
  *From Static to Dynamic: Benchmarking Real-World Code Review with MCR-Bench*
  ...
  👥 **作者**：Dewu Zheng, Yanlin Wang, Xiwen Wang, Kefeng Duan, Hongyu Zhang, Xilin Liu, Yuchi Ma, Zibin Zheng
  🏛️ **单位**：Sun Yat-sen University, Chongqing University, Huawei Cloud Computing Technologies Co., Ltd.
  📝 **摘要**：现有代码审查方法多将其简化为单轮静态决策，未能捕捉真实场景中多轮交互和复杂问题解决过程。本文提出MCR-Bench，首个缺陷状态感知的真实多轮代码审查基准。该基准涵盖五种常用编程语言，包含2,269个真实多轮代码审查任务，每个任务均标注细粒度缺陷信息（描述、类型、严重性）及跨轮次状态标签，记录缺陷在多轮过程中的完整演化轨迹。对主流大语言模型的实验发现：(1) 整体能力有限，缺陷检测和生命周期状态跟踪性能随交互轮次增加显著下降；(2) 性能对缺陷类型敏感，语义复杂或低显著性缺陷更易被遗漏；(3) 错误分析揭示了跨轮次时间错位和长程记忆不足等导致假阳性和假阴性的根本机制。
  🔗 [PDF](https://arxiv.org/pdf/2608.27442v1)
...
- **转导语言模型的随机估计**
  *Stochastic Estimation of Transduced Language Models*
  ...
  📝 **摘要**：转导语言模型（TLM）通过组合预训练源语言模型和有限状态转导器来诱导目标字符串的语言模型。计算目标前缀概率涉及对指数级或无限源字符串概率求和，现有基于阈值剪枝束搜索的方法仅提供误差未知的下界。本文提出一种无偏估计方法，通过无放回重采样源前缀，并根据其包含概率的倒数进行重加权。递归应用此校正可获得目标前缀概率的无偏估计，并能估计阈值剪枝丢失的概率质量。该算法通过扩展保留的源前缀并采样保留哪些前缀来减少计算量，保证以概率1终止。在百科文本和DNA数据上的评估显示，该方法在文本上实现了更好的计算-方差权衡，在DNA上以相同最大粒子数获得更低误差，并在DNA到氨基酸转导中比阈值剪枝束搜索减少数个数量级的运行时间。
  🔗 [PDF](https://arxiv.org/pdf/2608.27428v1)

- **通过弱模型引导增强RLVR中的LLM探索能力**
  *Boosting LLM Exploration via Weak-Model Guidance in RLVR*
  ...
  📝 **摘要**：基于可验证奖励的强化学习（RLVR）虽能提升大语言模型推理能力，但常导致策略熵下降，限制推理覆盖范围并降低大k值下的pass@k性能。现有方法多依赖算法正则化，忽视了跨模型非参数扰动。本文提出一种简单有效的方法，通过强制目标模型基于较小弱语言模型生成的部分推理轨迹来生成答案，以保留生成多样性。这些不熟悉的前缀能有效打破过度自信，鼓励探索不同的推理路径。实验揭示了分布差异对RLVR训练探索动力学的影响机制。在多个数学基准上的测试表明，该方法一致优于标准RLVR，且随着k值增大，性能增益愈发显著，证明其大幅扩展了推理覆盖范围。此外，该方法无需额外SFT、复杂奖励设计或提示工程即可有效缓解熵坍缩。
  🔗 [PDF](https://arxiv.org/pdf/2608.27420v1)

- **跨领域整合RLVR能力：深入探讨融合范式**
  *Consolidating RLVR Capabilities Across Domains: A Deep Dive into Fusion Paradigms*
  ...
  📝 **摘要**：基于可验证奖励的强化学习（RLVR）通常需训练独立领域专家再整合。本文按复用产物将三种融合范式分类：Merge（合并专家任务向量）、Mix RL（混合数据集）和MOPD（多教师在线策略蒸馏）。通过共享专家和数据，在不同模型规模和多领域基准上对比三者。结果显示，虽然平均性能差异不超过1.4分，但在单一基准上差距可达8.6分，且领域级变化与任务向量几何中的跨领域关系一致。训练动态揭示各自约束：Mix RL依赖领域混合比例，MOPD受限于教师模型，Merge将所有专家更新压缩为一个。三者均提升单样本准确率，但未显著增加解决方案覆盖范围或损失保留能力。研究提供了实用指南：已有专家且需低成本融合时用Merge；无专家需训练统一模型时用Mix RL；需保留特定领域增益时用MOPD。
...
- **语言模型如何组织和结构化道德知识**
  *How Language Models Organize and Structure Moral Knowledge*
  ...
  📄 `arXiv:2608.27402` · cs.CL, cs.AI, cs.LG
  👥 **作者**：Orion Reblitz-Richardson
  📝 **摘要**：本文探讨大语言模型如何组织道德知识，超越简单的道德内容检测，考察其是否区分道德基础并几何化组织其关系。通过在开源语言模型上训练六个独立的线性探针（对应道德基础理论的六个类别），研究发现这些方向既未坍缩为单一检测器，也未彼此隔离，而是跨越近最大数量的独立维度，同时共享一个正向公共组件。该共享组件是整合的标志，且相对于匹配的非道德概念电池具有道德特异性（平均成对余弦0.26 vs 0.013）。这种几何结构在不同架构和规模间保持一致，并在预训练早期达到整合状态。模型发现的结构未显示道德基础理论预测的个体化/结合区分，而是反映语料库统计。在道德困境中，每个困境方向部分由其组成基础构成，模型代表的是道德张力本身，而非预先解决的判断。
  🔗 [PDF](https://arxiv.org/pdf/2608.27402v1)

- **使临床语言模型可审计：概念引导的微调以实现稳健预测**
  *Making Clinical Language Models Auditable: Concept-Guided Fine-Tuning for Robust Prediction*

  📄 `arXiv:2608.27397` · cs.CL, cs.AI
  👥 **作者**：Jin Mu, Guanhua Chen
  🏛️ **单位**：University of Wisconsin–Madison
  📝 **摘要**：临床语言模型虽在医院内准确率高，但因利用笔记特定伪影（如模板、分隔符）而非患者状态，在部署偏移下易失效。本文提出CAST（概念引导伪影抑制微调），一种基于稀疏自编码器（SAE）的可审计临床文本分类框架。CAST利用SAE从Transformer中间激活中暴露稀疏、人类可审计的特征，通过LLM辅助解释管道和ICD-10检索约束对SAE潜在变量进行标记，在微调期间通过残差减法抑制已验证的伪影潜在变量，并提供事后逐概念归因以审计模型决策。在MIMIC-IV出院笔记死亡率预测任务上，CAST优于相应的微调编码器基线，并与强大的LLM基线保持竞争力，同时生成支持每个预测的临床概念及训练期间被抑制的伪影概念的特征级审计轨迹。
  🔗 [PDF](https://arxiv.org/pdf/2608.27397v1)
...
- **RATIO：科学文献中跨类型构思操作检索的基准**
  *RATIO: A Benchmark for Retrieval Across Typed Ideation Operations in Scientific Literature*
  ...
  📝 **摘要**：检索到的科学文献可为人类和AI科学家提供灵感，这种灵感可能表现为直接建议解决方案，或在不同抽象层级上呈现方向。本文提出RATIO（跨类型构思操作检索），一个大规模基准，其相关性由三种名为“构思移动”的操作定义：Address（检索针对陈述问题的潜在方法）、Broaden（检索更通用的表述）和Specify（检索具体实例化）。RATIO通过扩展话语标记远程监督至语料库级检索，并结合广泛的LLM和人工审核，从数百万篇计算机科学全文论文中构建。实验表明，操作特定的微调能显著提升检索器性能，但仍留有改进空间。RATIO为支持基于文献的构思的检索组件提供了可扩展的训练和评估框架，开辟了科学灵感检索的新研究途径。
...
- **CorporateBench：基于时间知识库的大规模问答基准测试**
  *CorporateBench: Large-Scale Q&A Benchmarking with Temporal Knowledge Bases*
  ...
  👥 **作者**：Sil Hamilton, Albert Yu Sun, Oscar J. Romero, Carl-Leander Henneking, David Mimno, Bishan Yang, Igor Labutov
  🏛️ **单位**：Epiq AI Labs, Cornell University
  📝 **摘要**：评估大语言模型在企业级文档集合上的复杂问答能力极具挑战，因公司内部通信难以共享且合成数据集过于简单。本文提出CorporateBench（CB），一个经人工验证的多任务问答基准，其规模接近LLM在企业通信网络中遇到的条件，评估语料库超过230,000份文档。CB通过四个从12人到10,000人的合成公司，在信息提取和知识库查询两个维度上评估LLM。每个语料库均从描述一致世界的时间演化知识库中采样，保证即使跨越数十万份文档，跨文档逻辑一致性依然成立。对五个LLM的评估揭示，随着输入规模接近现实水平，性能逐渐下降。CB为LLM开发者提供了企业通信推理的度量指标，填补了基准测试生态系统中的关键空白。
  🔗 [PDF](https://arxiv.org/pdf/2608.27391v1)
...
- **D2C-Routing：用于混合来源AI生成文本检测的维度到组合证据路由**
  *D2C-Routing: Dimension-to-Composition Evidence Routing for Mixed-Origin AI-Generated Text Detection*
  ...
  📝 **摘要**：AI生成文本检测通常被框定为二元文档级判断，但这在混合来源写作中失效，因为内容来源和表达来源可能不同。本文将混合来源检测重构为维度到组合的来源归因，先推断内容和表达来源，再将其组合为四种协作类型。提出D2C-Routing方法，将内容侧和表达侧证据路由到监督维度头，然后通过学习的门控组合层预测最终标签。在基于HART混合来源基准重构的MixD2C数据集上，D2C-Routing检测系统达到0.8603的四路平均TPR@1%FPR，比同一分割的RACE-local重跑高6.5分。核心消融实验支持路由设计，错误分析表明区分AI内容/人类表达与完全AI生成文本仍是最难的边界。
...
- **Puro-2B：在RTX 5090上以5090美元以内训练的低成本Qwen2-1.5B**
  *Puro-2B: Poor Lab's Qwen2-1.5B Trained on RTX 5090 within $5090*
  ...
  👥 **作者**：Kairong Luo, Jiarui Cui, Yaorui Yin, Shengqi Chen, Yiming Yang, Linxiang Gao, Yanmohan Wang, Mingzhe Zhang, Kaiyue Wen, Kaifeng Lyu, Wenguang Chen
  🏛️ **单位**：Tsinghua University, Pengcheng Laboratory
  📝 **摘要**：语言模型预训练通常成本高昂，本文提出一种开源预训练配方，旨在降低这一门槛。利用该配方，在消费级RTX 5090 GPU上以FP8精度从头训练Puro-2B模型系列，最高使用1.4万亿Token。最佳模型训练成本低于6.9K美元，性能接近Qwen2.5-1.5B。这种成本效率得益于硬件选择、低精度训练、超球优化、课程模型平均和数据配方的结合。此外，本文推导了Puro成本缩放定律，表明约4.4K美元即可达到Qwen2-1.5B的性能。作为端到端案例研究，还考察了预训练数据课程如何塑造后训练后的下游性能。所有训练配方、数据、代码和模型权重均以Apache 2.0协议发布。
  🔗 [PDF](https://arxiv.org/pdf/2608.27370v1)
...
- **你的语音克隆系统其实是一个语音匿名化工具**
  *Your Voice Cloning System is Secretly a Voice Anonymizer*
  ...
  📝 **摘要**：说话人匿名化旨在抑制语音中的说话人识别属性，同时保留语言内容和质量。本文提出无需重新训练即可将XTTSv2（一个在2.7万小时语音上训练的多语言语音克隆模型）重新用于说话人匿名化。关键洞察在于XTTSv2的语音克隆能力能独立于说话人身份保留韵律结构，从而通过条件化于伪说话人实现语音转换。引入一种迭代细化策略，通过最大化说话人差异性和可懂度的调和平均来平衡隐私和效用。在CommonVoice和Multilingual LibriSpeech上的七种欧洲语言评估中，该系统实现了接近最优的隐私（EER≈0.49）、具有竞争力的可懂度，以及比专用匿名化基线显著更好的语音质量，且无需特定语言训练。
...
- **UrbanGround：从局部感知到真实尺度城市中的空间能动性**
  *UrbanGround: From Local Perception to Spatial Agency in a Real-Scale City*
  ...
  📝 **摘要**：本文提出UrbanGround，首个基于全港3D地理空间数据构建的真实尺度城市沙盒，旨在评估多模态大语言模型（MLLM）智能体如何将局部视觉感知转化为可靠的空间行动。该环境支持第一人称闭环交互及交互式地图导航。研究通过三个递进问题展开：首先测试智能体在主动观察后能否准确回答空间问题；其次考察随着目的地变远且描述模糊，这种定位能力是否支持导航；最后验证行为在路线变化和行人动态下的鲁棒性。实验发现，当前MLLM智能体在视觉识别和短程空间推理上具备原子能力，但在方向感和行人感知运动方面不可靠。其核心失败在于长程探索中，局部能力无法组合成持续的目标导向行为，且错误累积缺乏有效纠正机制。
  🔗 [PDF](https://arxiv.org/pdf/2608.27456v1)

- **检索头遇见视觉：揭示视觉语言模型如何定位和提取视觉信息**
  *Retrieval Heads Meet Vision: Uncovering How VLMs Locate and Extract Visual Information*
  ...
  📝 **摘要**：本文受大语言模型中“检索头”启发，探究视觉语言模型（VLM）内部是否存在类似的视觉检索机制。研究提出“视觉检索头”（VRHs），即负责将文本描述因果性地定位到图像区域的一小撮注意力头（约占1.7-2.6%）。通过统一的设计空间重新审视现有头评分方法，发现基于输出预测token对真实指代区域求和的注意力评分能最可靠地识别因果头。在11个VLM和5个指代表达基准上，仅屏蔽前20个VRHs可使定位准确率下降高达80个百分点，而屏蔽相同数量的随机头影响甚微。此外，VRHs展现出跨任务泛化性、功能特异性（破坏定位但保留格式）以及架构共享性（在共享LLM骨干但视觉编码器不同的VLM间可迁移），揭示了VLM视觉定位的稀疏因果机制。
...
- **利用大型重建模型重构交互中的人体与物体**
  *Reconstructing Humans and Objects in Interaction using Large Reconstruction Models*
  ...
  📝 **摘要**：针对3D人体-物体交互（HOI）重建中存在的深度歧义、遮挡和物体形状多变等挑战，本文提出MILO框架，利用大型重建模型（LRM）从单张图像恢复详细的3D HOI。现有方法主要依赖重投影和接触约束拟合参数化模型，而MILO的关键观察在于LRM提供了保留人体-物体相对排列和邻近线索的强大几何支架。该方法将重建问题重构为对LRM网格的解释：将其分割为人体和物体组件，对人体部分拟合参数化身体模型，并在可用时对齐物体模板。实验表明，MILO在多个基准和交互场景中实现了强大的重建精度，优于现有基线方法，为AR/VR、机器人和具身AI应用提供了新的技术路径。
  🔗 [PDF](https://arxiv.org/pdf/2608.27407v1)

- **CLAP：跨具身视频世界模型即零样本物理模拟器**
  *CLAP: Cross-Embodiment Video World Models are Zero-Shot Physical Simulators*

  📄 `arXiv:2608.27406` · cs.RO, cs.AI, cs.CV
  👥 **作者**：Kechen Liu, Ola Shorinwa
  🏛️ **单位**：Princeton University
  📝 **摘要**：现有动作条件视频模型通常局限于单一机器人具身，无法利用包含丰富物理信号的异构互联网视频数据。本文提出CLAP，一个跨具身动作条件视频生成框架，旨在从人类和机器人视频中学习通用物理。CLAP通过末端执行器位姿、语言指令和潜在动作协调不同的动作空间，并引入基于课程学习的跨具身训练策略：先利用潜在动作从无标签视频学习基础物理先验，再将其锚定到末端执行器动作空间以实现零样本部署。在DROID等挑战性环境中，CLAP性能接近或超越最先进的单具身模型，并通过少样本适应进一步提升。CLAP提供了迄今最全面的动作条件视频世界模型套件，涵盖多种动作空间和机器人形态，并开源了代码和模型。
  🔗 [PDF](https://arxiv.org/pdf/2608.27406v1)
...
- **LeVJEPA：无启发式的高效可扩展视频预训练**
  *LeVJEPA: Efficient & Scalable Video Pretraining without the Heuristics*
  ...
  👥 **作者**：Lukas Kuhn, Lucas Maes, Giuseppe Serra, Quentin Le Lidec, Yann LeCun, Randall Balestriero, Florian Buettner
  🏛️ **单位**：German Cancer Research Center, German Cancer Consortium, Goethe University Frankfurt, Mila, Université de Montréal, Brown University, Courant Institute New York University, Advanced Machine Intelligence (AMI Labs)
  📝 **摘要**：视频蕴含物理世界的时序结构，但现有自监督方法计算成本高昂，常依赖架构不对称或像素重建来防止表示坍缩。本文提出LeVJEPA，首个在LeJEPA无坍缩目标下训练的视频编码器，摒弃了上述启发式方法。LeVJEPA仅使用单个编码器和投影器，通过SIGReg正则化保证无坍缩，目标函数简化为单一超参数。其核心优势在于：通过均匀随机token丢弃降低预训练计算量，同时提升下游精度；在相同数据下，以5.6至20.8倍更少的计算量匹配或超越V-JEPA 2。此外，由于无需分支不对称，编码器可采用块因果注意力训练，使时序成为编码器固有属性。实验表明，去除计算开销后，视频成为通用视觉预训练的可行且优越基底。
  🔗 [PDF](https://arxiv.org/pdf/2608.27395v1)
...
- **连续容量增长：JEPA世界模型中视觉Transformer编码器的任务复杂度驱动宽度与深度扩展**
  *Successive Capacity Growth: Task-Complexity-Driven Width and Depth Expansion for Vision Transformer Encoders in JEPA World Models*
  ...
  👥 **作者**：Frederik Berenz
  🏛️ **单位**：121-labs.com
  📝 **摘要**：JEPA世界模型通常使用固定大小的ViT编码器，导致简单任务资源过剩而复杂任务不足。本文提出连续容量增长（SCG）方法，从最小编码器（1头、2层、283K参数）开始，根据任务复杂度动态扩展宽度（增加注意力头）或深度（增加Transformer块）。SCG利用任务无关的测试-验证机制和功能保持扩展，安全地尝试架构变更并在未改善预测损失时回滚。SIGReg确保学习到的语义维度保持统计独立，防止架构增长过程中的坍缩。实验显示，在60维多物体动力学任务中，SCG触发深度扩展，预测损失比固定小基线改善20.3%，参数效率比固定大模型高56倍；在2D导航任务中，单次宽度扩展即比固定大模型改善23%。结果表明，JEPA编码器无需预分配最大容量，可随任务需求逐步增长。
  🔗 [PDF](https://arxiv.org/pdf/2608.27367v1)

- **KnockGS：基于交互校准的物理高斯表示**
  *KnockGS:interaction-Grounded Calibrationof Physical Gaussian Representations*
  ...
  📝 **摘要**：现有物理集成3D高斯表示通常假设材料参数已知或手动指定，限制了其在参数需从动态中推断时的应用。本文提出KnockGS，一个基于交互响应的PhysicalGS框架，通过已知施加力下的物体动态估计3D高斯物体的弹性与密度尺度。该方法将力诱导响应转化为校准信号：从观察到的动态中提取时序响应特征，估计两个材料尺度，并将冻结的估计值写回模拟器，以测试其在未拟合交互上的预测能力。在参数恢复和响应保真度评估中，KnockGS在五个保留材料目标上比响应检索、全局回归或固定默认材料更准确地恢复尺度，且冻结估计在不同方向和量级的交互下仍具预测性。该研究是迈向渲染外观与模拟响应一致的交互式PhysicalGS系统的第一步。
...
- **PAWBench：我们距离概率对齐的世界建模还有多远？**
  *PAWBench: How Far Are We from Probabilistically Aligned World Modeling?*
  ...
  📝 **摘要**：视频生成模型常被视作世界模型，但现有评估主要关注单视频合理性，未测试重复生成是否能恢复正确的行为分布。本文形式化了“概率对齐”这一分布级标准，即世界模型应复现相同初始观察和动作下可能行为的分布。为此，引入PAWBench基准，将视频生成器评估为世界动态的随机采样器，并提出PAWEval协议，将重复视频滚动转换为物理行为的经验分布。在50个场景和11个当前系统上的测试显示，没有模型能一致地匹配参考概率并恢复有效行为范围。研究进一步测试了语言提示、初始噪声采样或模型训练是否能重塑预测分布。该工作为迈向概率对齐的世界建模奠定了基础，揭示了当前视频生成器在分布建模上的显著差距。
...
- **R2M-Bench：通过相对一致性评估交互式视频世界模型中的重访记忆**
  *R2M-Bench: Evaluating Revisit Memory via Relative Consistency in Interactive Video World Models*
  ...
  📝 **摘要**：首次访问与返回帧的高相似度并不一定表明视频世界模型记住了场景，中间滚动可能变化甚微。这种歧义使绝对重访分数对渲染稳定性和重复内容敏感。本文提出R2M-Bench，一个基于可观察重访选择性一致性的基准。对于每个检测到的返回，R2M-Bench将重访对与同一滚动中的两个对照进行比较：间隙匹配的非重访对（测量通用时间稳定性）和短程对（估计短地平线一致性）。由此产生记忆增益（MG）和归一化记忆比（NMR）。基准包含100个参考场景和300个实例，评估外观保真度、场景/物体身份、局部几何和持久状态。在7个动作条件视频世界模型中，总体NMR与人类一致性判断呈显著相关（Spearman's ρ=0.547），且相对校准大幅减少了慢动作捷径。DreamX-World-Memo在评估模型中取得了最高的总体NMR。
...
- **用于实验室物体沉浸式可视化的3D重建方法比较评估**
  *Comparative Evaluation of 3D Reconstruction Methods for Immersive Visualization of Laboratory Objects*
  ...
  📝 **摘要**：本研究旨在评估当前3D重建方法是否支持创建用于教育目的的真实实验室物体全息表示。比较了摄影测量、基于神经辐射场（NeRF）的方法、高斯泼溅和LiDAR四种技术，生成常见实验室物品的全息模型，并由研究生通过重复测量设计评估其形状、颜色、纹理和视觉缺陷。结果显示，NeRF-based方法产生了最一致的高保真表示，特别是在透明、反射或低纹理物品上表现优于其他方法。形状和颜色的重现通常比纹理更成功，表明某些视觉属性在教育全息图中仍具挑战性。该研究不仅识别了各重建方法的优缺点，还展示了一种创建沉浸式学习物体的实用工作流，支持AR/MR教育环境中的实验前准备、空间推理和学生参与度，为教育工作者和研究人员提供了设计洞察。
...
- **从高分辨率航空影像中检测圣诞树种植园：法国Morvan案例研究**
  *Detection of Christmas tree plantations from high-resolution aerial imagery. A case study in the French Morvan*
  ...
  📝 **摘要**：圣诞树种植园在遥感中是一个未被充分探索的领域，其边界划定面临高种植密度、短轮伐期、与周围植被视觉混淆及严重类别不平衡等挑战。本文将圣诞树种植园映射框架化为独特的稀有目标语义分割问题，并提出硬负样本挖掘（HNM）策略以改善对混淆背景模式的判别能力。在2020年测试集上，最佳模型DeepLabV3（ResNet-34编码器）实现了0.733的IoU和0.846的F1分数。HNM显著提升了精确率-召回率行为，将PR曲线下面积从0.204提高至0.913。时间推断显示良好的可迁移性，在2017/2018和2023年分别达到0.751/0.858和0.691/0.817的IoU/F1值。大规模验证突显了任务的内在难度，圣诞树种植园仅占评估区域总面积的1.72%至2.04%。
...
- **TADP：用于单阶段3D物体检测的任务感知可变形预测**
  *TADP: Task-Aware Deformable Prediction for Single-Stage 3D Object Detection*
  ...
  📝 **摘要**：大多数单阶段3D物体检测器使用相同的提取特征完成不同任务，但无法将特征投影到适应所有任务的公共空间。本文提出任务感知可变形预测（TADP）方法以解决此问题。首先，设计三重特征细化聚合模块自适应提取三级特征；其次，设计多尺度特征聚合块以尺度感知方式融合多尺度特征；最后，利用即插即用的任务感知变形头对每个任务的预测进行变形，以感知各任务的强调和交互。实验结果表明，该变形头在其他检测方法上表现良好。在KITTI数据集上，TADP实现了80.91%的汽车mAP，超越了KITTI基准上的许多最先进方法，证明了其在单阶段3D检测中的有效性和优越性。
...
- **Sidecar：用于角色一致自由形式视觉叙事的无训练语义复用**
  *Sidecar: Training-Free Semantic Reuse for Character-Consistent Free-form Visual Storytelling*
  ...
  📝 **摘要**：视觉叙事要求生成遵循叙事且跨帧保持角色身份一致的图像。在自由形式故事生成中，角色仅在首次引入时完全描述，后续通过类型级提及或代词指代，导致后续提示可能遗漏关键身份语义，增加一致性维护难度。本文提出Sidecar，一个即插即用的语义增强模块，保留初始描述中的实体级信息，并将缺失语义注入后续提示嵌入中。Sidecar无需额外训练，也不修改基础扩散模型架构。在FreeStoryBench上的实验表明，Sidecar在多个基于SDXL和FLUX的基线上持续改善了提示-图像对齐和角色一致性，且计算开销可忽略不计。该方法为自然叙事场景下的角色一致性生成提供了高效解决方案。
...
- **用于大规模全切片图像嵌入提取的解耦I/O主导流水线**
  *Decoupled I/O-Dominant Pipelines for Large-Scale Whole-Slide Image Embedding Extraction*
  ...
  📝 **摘要**：全切片图像（WSI）是计算病理学的核心，但其巨大尺寸使得基于补丁的处理成为基础模型推理的实际单元。在大规模场景下，生成和处理海量补丁会引入显著的I/O和编排开销，往往主导端到端性能。本文提出一种解耦的、I/O感知的流水线，将工作流分解为三个阶段：补丁生成与暂存、可尴尬并行嵌入推理、分片向量数据库摄取。该设计将数据移动与计算隔离，实现高效补丁交付和最小通信的多节点可扩展推理。系统生成分布式向量数据库，嵌入与丰富元数据（如患者、切片、补丁属性）持久耦合，支持高效过滤、检索和下游复用。实验表明，解耦I/O、计算和摄取可实现高吞吐量WSI嵌入提取，并证明存储在中高并发下占主导地位，将WSI嵌入提取重新定义为数据为中心的系统问题。
...
- **UniFLM：胎儿肢体超声图像的统一分割与测量**
  *UniFLM: United Segmentation and Measurement on Fetal Limb Ultrasonic Image*
  ...
  📝 **摘要**：产前超声检查对评估胎儿肢体发育和检测先天异常至关重要，但现有AI模型常因缺乏高质量标注数据和统一框架而忽略胎儿致死性骨骼发育不良。本文构建了胎儿肢体骨骼（FLB）数据集，包含肱骨、股骨、胫腓骨和桡尺骨的高质量标注。提出UniFLM，一个用于自动跨平面分割和测量的统一框架。UniFLM整合了语义感知跳跃连接模块以桥接编码器和解码器特征间的语义差距，正采样策略以自适应过滤噪声并提取关键语义信息，以及点回归映射模块以学习临床医生标注模式实现精确骨长测量。在FLB数据集上的广泛实验表明，UniFLM在胎儿长骨评估中相比当前最先进模型实现了更高的精度和更强的泛化能力。
